...
// Fetches a player's profile from the Brawl API and normalizes it into
// FormattedPlayerInfo. Throws on a non-ok response — callers decide how to
// surface it (HTTP 502 for the route; invocation error for the Lambda).
// Shared by the Express controller and the backend-invoke Lambda handler.
export async function fetchAndFormatPlayer(
  playerTag: string,
): Promise<FormattedPlayerInfo> {
